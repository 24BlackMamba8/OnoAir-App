...
@Component({
  ...
  templateUrl: './last-minute-bookings.component.html',
  styleUrls: ['./last-minute-bookings.component.css'],
  standalone: true,
  imports: [
    FormsModule,
    MatFormFieldModule,
...
export class LastMinuteBookingsComponent implements OnInit {
 ...
 // ✅ משתנה לשליטה במעבר השלבים
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<LastMinuteBookingsComponent>
    ) {
  ...
  editBooking(order: any): void {
    const dialogRef = this.dialog.open(LastMinuteBookingsComponent, {
      width: '800px',
      data: {
        flight: order.flight,
